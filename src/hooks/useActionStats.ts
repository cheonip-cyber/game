...
export function useActionStats() {
  ...
  const addPoints = (points: number) => setGameState((s) => ({ ...s, actionPoints: s.actionPoints + points }));
  const upgradeStat = (stat: keyof RoutineLevels, cost: number) => {
    if (gameState.actionPoints < cost) return false;
    setGameState((s) => ({ ...s, actionPoints: s.actionPoints - cost, routineLevels: { ...s.routineLevels, [stat]: s.routineLevels[stat] + 1 } }));
    return true;
  };
