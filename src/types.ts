export interface UpgradeState {
  maxHpLevel: number; // Max HP +10% per level
  speedLevel: number; // Speed +5% per level
  damageLevel: number; // Damage +10% per level
  magnetLevel: number; // Magnet Range +15% per level
  dashLevel: number; // Dash cooldown -10% per level
}
